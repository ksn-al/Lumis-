...
export const forgotPassword = async (req: Request, res: Response) => {
  ...
  await sendEmail(
    email,
    'Password Reset',
`<p>To reset your password, click the link: <a href="${resetLink}">${resetLink}</a></p>`
  );
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  const user = await prisma.user.findUnique({ where: { email }, select: { resetToken: true, email: true } });
