...
function Messages() {
  ...
  const [nextCursor, setNextCursor]                   = useState(null);  // ISO timestamp

  const socket             = useSocket();
  const selectedConvRef    = useRef(null);   // stale-closure-safe ref
  const messagesEndRef     = useRef(null);   // auto-scroll anchor
  const messagesListRef    = useRef(null);   // container for scroll-position preservation
  const suppressScrollRef  = useRef(false);  // true while prepending older messages
  ...
  // Auto-scroll to bottom — suppressed while loading older messages
  useEffect(() => {
  ...
  // ── Real-time messages via shared socket ──────────────────────────────────
  useEffect(() => {
    if (!socket) return;
  ...
  // ── Auto-select conversation when navigating via ?to= ─────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        setMessages(data.messages ?? data);          // handle both old and new shape
        setNextCursor(data.nextCursor ?? null);

        // Mark messages as read (best-effort — don't block the UI on failure)
  ...
  const loadOlderMessages = useCallback(async () => {
    ...
    try {
      ...
      setMessages(prev => {
        // Dedup — skip any messages already in state
        const existingIds = new Set(prev.map(m => m.id));
        return [...older.filter(m => !existingIds.has(m.id)), ...prev];
      });
      setNextCursor(data.nextCursor ?? null);

      // Restore scroll position so viewport stays on the same message
