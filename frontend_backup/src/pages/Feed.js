...
function Feed() {
  ...
  // IDs of posts already rendered — used to dedup real-time arrivals
  const renderedIds = useRef(new Set());

  // ── Initial feed load + current user id ────────────────────────────────
  useEffect(() => {
  ...
  // ── Real-time new posts via shared socket ─────────────────────────────
  useEffect(() => {
    if (!socket) return;
  ...
  const handleCreatePost = async (e) => {
    ...
    try {
      ...
      if (data.post) {
        // Own new post — add to top immediately (socket won't echo it back to author)
