...
export default function Sidebar() {
  ...
  // Fetch user info + initial badge counts once on mount
  useEffect(() => {
    apiRequest('/users/me')
      .then(data => {
        if (!data.user) return;
  ...
  useEffect(() => {
    ...
    const handleNotif = (notif) => {
      // new_message is tracked by the messages badge, not notifications
  ...
  // Reset badges when user navigates to the relevant page
  useEffect(() => {
